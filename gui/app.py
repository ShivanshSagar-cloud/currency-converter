    ...
    def _load_currencies_async(self) -> None:
        def worker() -> None:
            try:
                ...
                # Capture the message as a plain string NOW. `error` is deleted
                # by Python at the end of this except block, so a lambda that
                # closes over `error` itself would raise NameError once it
    ...
    def convert_async(self) -> None:
        ...
        def worker() -> None:
            try:
                ...
                # Same fix as above: snapshot the message into a plain string
                # before scheduling the lambda via self.after().
                error_message = str(error)
                self.after(0, lambda: self._show_error(error_message))
            except Exception as error:  # noqa: BLE001
